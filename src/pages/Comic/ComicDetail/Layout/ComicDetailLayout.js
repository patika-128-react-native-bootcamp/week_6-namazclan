...
export default function ComicDetailLayout({details, onPress, charactersData}) {
  const source =
    details.images[0] !== undefined
      ? `${details.images[0].path}.jpg`
      : 'https://legacycomics.com/wp-content/uploads/2019/07/marvel-comics-1000-allred-60s.png';
  const detail =
    details.textObjects[0] == undefined
      ? emptyText
      : details.textObjects[0].text;
      ...
      <ImageBackground
        ...
        <View style={styles.innerContainer}>
          ...
          <Text style={styles.descpription}>
            {detail && detail.split('<br>', 1)}
          </Text>
          <Text style={styles.pageCount}>Page Count: {pageCount}</Text>
          <FlatList data={charactersData} renderItem={renderItem} />
        </View>
      </ImageBackground>
    </SafeAreaView>
  );
